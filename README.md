...
* DAG 任务编排：任务之间通过强依赖 / 弱依赖自动形成有向无环图，避免手工管理复杂的执行顺序。
* 强依赖 / 弱依赖：
  * 强依赖：必须等待上游完成后再执行。
  * 弱依赖：可设置超时，总预算或逐依赖等待，灵活平衡启动速度与依赖完整性。
* 任务输入聚合：下游任务可自动接收多个上游的执行结果，支持单输入 / 多输入（List）/ 无输入（Unit）。
* 任务类型隔离：支持 IO 密集型、CPU 密集型任务分开调度，减少线程池资源竞争。
* 优先级调度：任务支持设置优先级（数字越小优先级越高），可实现关键任务优先执行。
* 取消传播：支持取消整个 DAG 或单个任务，自动传播取消信号，避免不必要的计算。
* 失败恢复：
  * 失败重试（支持最大重试次数与间隔配置）。
  * 回滚操作（可选）。
  * 失败回调。
* 协程原生实现：基于 Kotlin 协程与 Channel，天然支持异步、背压、取消语义。
* 任务结果等待：所有任务都暴露为 Deferred，可选择 await 方式等待某个任务的结果。
* 日志与可观测性：调度器在关键事件点（入队、执行、依赖完成、失败、回滚、取消）均有日志输出，方便调试与监控。
* 动态 DAG 支持：运行时可按需添加新任务（只要不破坏 DAG 的无环性）。
